...
def main():
    ...
    seq2seq.load_state_dict(torch.load('checkpoint_generator_1/seq2seq_save_model1.pt'))  # TODO: 10.7
    seq2seq.to(device)
    ...
    for i in range(EPOCHS):
        ...
        for _, batch in enumerate(train_iter):
            ...
            inp = src
            dec_out = trg
            dec_inp = torch.cat((torch.ones(size=[batch_size, PAD_IDX]).long().to(device), trg[:, 0:-1].to(device)),
                                dim=1)  # maybe wrong
            # train_seq2seq
        ...
        if True:  # i%1 == 0:
            ...
            for _, batch in enumerate(dev_iter):  # for _ in range(1, num_batches + 1):  word, char, pos, heads, types, masks, lengths = conllx_data.get_batch_tensor(data_dev, batch_size, unk_replace=unk_replace)  # word:(32,50)  char:(32,50,35)
                ...
                inp = src
                dec_out = trg
                sel, _ = seq2seq(inp.to(device), LEN=inp.size()[1]+5)
