    ...
    for i in range(EPOCHS):
        ...
        if True:  # i%1 == 0:
            ...
            for _, batch in enumerate(
                    dev_iter):  # for _ in range(1, num_batches + 1):  word, char, pos, heads, types, masks, lengths = conllx_data.get_batch_tensor(data_dev, batch_size, unk_replace=unk_replace)  # word:(32,50)  char:(32,50,35)
                inp = batch.src
                # inp, _ = seq2seq.add_noise(word, lengths)
                dec_out = word
                sel, _ = seq2seq(inp[0], LEN=inp[0].size()[1])
...
if __name__ == '__main__':
    ...
    src_field = data.Field(sequential=True, tokenize=tokenizer, lower=True, fix_length=150, include_lengths=True, batch_first=True)  #use_vocab=False   fix_length=20,
    trg_field = src_field
    ...
    seq2seq_save_path = 'v_checkpoint'
